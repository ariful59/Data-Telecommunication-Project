...
public class Sender_head {
	...
	private String dh(String str){
		...
		if(Save_input.SAVE_DATALINK_SCHEME == 1){ //CRC-32 selected
			CRC C = new CRC();
			str = C.encode(str);
		}
		else if(Save_input.SAVE_DATALINK_SCHEME == 0){
	...
	private String phh(String str){
		...
		String Block = "";
		if(Save_input.SAVE_BLOCK_CODING == 0){
			FourBFiveB Four = new FourBFiveB();
			System.out.println(str.length());
			Block = Four.encode(str);
		}
		if(Save_input.SAVE_PHYSICALLINK == 0){
			NRZ_L nrz_l = new NRZ_L();
			str = nrz_l.encode(Block);
		}
		if(Save_input.SAVE_PHYSICALLINK == 1){
			NRZ_I nrz_i = new NRZ_I();
			str = nrz_i.encode(Block);
		}
		if(Save_input.SAVE_PHYSICALLINK == 2){
			RZ rz = new RZ();
			str = rz.encode(Block);
		}
		if(Save_input.SAVE_PHYSICALLINK == 3){
			Manchester man = new Manchester();
			str = man.encode(Block);
		}
		if(Save_input.SAVE_PHYSICALLINK == 4){
